...
export default function Index({ userData, setFilteredUserData }) {
  ...
  const handleSortByUser = () => {
    let sortedData = [...userData].sort((a,b) => (a.firstName > b.firstName) ? 1 : -1);
    setFilteredUserData(sortedData);
  }

  const handleSortByRole = () => {
    let sortedByName = [...userData].sort((a,b) => (a.firstName < b.firstName) ? 1 : -1);
    let sortedData = [...sortedByName].sort((a,b) => (a.isAdmin < b.isAdmin) ? 1 : -1);
    setFilteredUserData(sortedData);
  }

  const handleSortByStatus = () => {
    let sortedByName = [...userData].sort((a,b) => (a.firstName < b.firstName) ? 1 : -1);
    let sortedData = [...sortedByName].sort((a,b) => (a.isActive < b.isActive) ? 1 : -1);
    setFilteredUserData(sortedData);
  }
